...
const ChildrenPage = async () => {
  ...
  // Filter products to display only those from the 'Children' category
  const childrenProducts = products.filter((product) => product.category?.name.toLowerCase() === 'children');

  // Log the filtered products
  console.log('Filtered children products:', childrenProducts);
  ...
  return (
    <div className="container mx-auto mt-24 px-4 sm:px-6 md:px-8 lg:px-12">
      <h1 className="text-5xl font-extrabold text-center text-green-600 mb-12 leading-tight tracking-wide">
        Discover the Best of <span className="text-black">Children&apos;s Fashion</span>
      </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {childrenProducts.length > 0 ? (
            ...
            <p className="col-span-full text-center text-lg text-gray-600">
              No products available in the Children&apos;s category.
            </p>
          )}
        </div>
      )}
